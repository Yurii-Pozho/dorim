...
def get_prices(organization):
    ...
    prices_dict = {
        ...
        'Astra Zeneca': {
            ...
            "Золадекс имп. спрол. всвоб. для подкож. введ. 3,6мг №1":	815958,
            "Нексиум таб. 20мг №14":	90053,
            "Нексиум таб. 40мг №14":	115488,
            "Пульмикорт сусп 0,25мг/2мл№20":	275027,
            "Пульмикорт сусп 0,5мг/2мл№20":	340220,
            "Симбикорт турбухалер пор. 160/4,5мкг/60доз№1":	281134,
            "Симбикорт турбухалер пор.80/4,5мкг/60доз№1":	253408,
...
def process_meros_excel(file_path, organization):
    ...
    df['Дата'] = pd.to_datetime(df['Дата'], format='%d.%m.%Y', dayfirst=True, errors='coerce').dt.date
    ...
    def get_inn_from_json(client_name):
        ...
        return inn_data.get(client_name, 309872412)
